...
const prefixes = ["Aether", "Amber", "Arc", "Astral", "Aurora", "Binary", "Blazing", "Hard", "Cipher", 
  ...
  "Velvet", "Void", "Zenith", "Gossamer", "Chrome", "Pedro", "Wild", "Maricarmen's", "Gravity", "Saffron", 
  ...
  "Glimmering", "Turtle", "Horny", "Storm", "Irene", "Golden", "Ultra", "Child", "Candy", "Donald", "Mia", "Gambling", "Trans", "Gay", "Lesbian", "Diddy", "Asshole", "Santi"];
...
const species = [
  ...
  "Toucan", "Lizard", "Love", "Trump", "Khalifa", "Addict", "Annihilator", "Party", "Fucker", "Porn", "Stalker", "Licker", "Eguren"];
...
for (let number = 1; number <= TARGET_COUNT; number += 1) {
  ...
  const baseChanceLog = 2.7 + RARITY_STEP * (number - 1);
  const baseChance = chanceFromLog(baseChanceLog);
let prefixIndex = hash(id, "prefix") % prefixes.length;
let speciesIndex = hash(id, "species") % species.length;

let prefix = prefixes[prefixIndex];
let speciesName = species[speciesIndex];
let name = `${prefix} ${speciesName}`;

// Guarantee that every prefix + species combination is unique.
while (names.has(name)) {
  ...
  if (prefixIndex >= prefixes.length) {
    throw new Error(
      `No unique name combination remains for alien ${id}.`
    );
  }

  prefix = prefixes[prefixIndex];
  speciesName = species[speciesIndex];
  name = `${prefix} ${speciesName}`;
}
...
byId.set(id, {
  ...
  prefix,
  species: speciesName,
  emoji: pick(emojis, id, "emoji"),
  rarity: rarity.name,
  color: rarity.color,
  baseChance,
  baseChanceLog: Number(baseChanceLog.toFixed(6)),
  baseIncome: Number(
    (2 * (10 ** ((baseChanceLog - 2.7) * 0.30))).toPrecision(10)
  )
});
}
...
console.log(
  `Wrote ${catalog.length.toLocaleString()} immutable alien definitions to ${output}`
);